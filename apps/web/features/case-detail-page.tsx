...
export function CaseDetailPage({ id }: { id: string }) {
  ...
  function submitComment(event: React.FormEvent) {
    ...
    addComment({
      id: `comment-${Date.now()}`, caseId: item.id, authorId: isExpert ? "expert-user-kim" : role === "admin" ? "admin-user" : "user-demo", author: isExpert ? "김수리" : role === "admin" ? "수리온 운영자" : role === "questioner" ? item.author : "경험나눔", role: isExpert ? "BUSINESS_EXPERT" : role === "admin" ? "ADMIN" : role === "questioner" ? "QUESTIONER" : "USER", expertId: isExpert ? "expert-kim" : undefined, type: commentType, body: commentBody.trim(), createdAt: "방금", replyToCommentId: replyTo?.id, replyToLabel: replyTo ? `${replyTo.author}님의 의견` : undefined, validExpertAnswer: isExpert && ["EXPERT_OPINION", "REQUEST_INFORMATION", "SAFETY_WARNING"].includes(commentType) && commentBody.trim().length >= 30, helpfulCount: 0,
    });
    setCommentBody(""); setReplyTo(null); setNotice("의견이 공개 대화에 등록됐어요.");
  }
      ...
      <div className="container case-layout">
        <article className="case-main">
          <header className="case-header">
            <div className="case-labels"><StatusBadge status={item.status} /><Link href={`/category/${categorySlug}`}>{item.category}</Link>{item.modelIdentificationStatus === "unknown" && <span className="unknown-model"><Info />모델 확인 필요</span>}</div>
            <h1>{item.title}</h1>
          ...
          {item.status === "RESOLVED" && <section className="resolution-card"><span className="resolution-check"><CheckCircle2 /></span><div><span className="eyebrow">해결 결과</span><h2>디스플레이 연결 케이블 문제를 해결했어요</h2><p>전문가의 안내로 원인을 좁힌 뒤 케이블을 교체했고 현재 정상 작동합니다.</p><dl><div><dt>해결 방식</dt><dd>{item.solvedBy ?? "직접 해결"}</dd></div><div><dt>실제 비용</dt><dd>45,000원</dd></div><div><dt>걸린 기간</dt><dd>5일</dd></div><div><dt>현재 상태</dt><dd>정상 작동</dd></div></dl><button><ThumbsUp />이 사례가 도움이 됐어요 <strong>{item.helpful}</strong></button></div></section>}

          <section className="conversation" id="conversation">
            <div className="conversation-head"><div><span className="eyebrow">공개 대화</span><h2>의견 {thread.length || item.comments}개</h2><p>질문자, 전문가, 일반 사용자가 함께 원인을 좁혀갑니다.</p></div><div className="role-switcher" title="데모 역할 전환"><label>보기 역할<select value={role} onChange={(event) => setRole(event.target.value as typeof role)}><option value="questioner">질문자</option><option value="expert">전문가</option><option value="user">일반 사용자</option><option value="admin">관리자</option></select></label></div></div>
            ...
            <div className="timeline">
              ...
              )) : <div className="no-comments"><MessageCircle /><strong>아직 의견이 없어요</strong><p>첫 번째 경험이나 전문가 의견을 남겨주세요.</p></div>}
            </div>

            <form className="comment-form" id="comment-form" onSubmit={submitComment}>
              <div className="comment-form-head"><div><span className="avatar">{role === "expert" ? "김" : role === "admin" ? "관" : "민"}</span><strong>{role === "expert" ? "김수리" : role === "admin" ? "수리온 운영자" : item.author}</strong><RoleBadge tone={role === "expert" ? "business" : role === "admin" ? "admin" : role === "questioner" ? "questioner" : "user"}>{role === "expert" ? "사업자 인증 전문가" : role === "admin" ? "관리자" : role === "questioner" ? "질문자" : "일반 사용자"}</RoleBadge></div><select value={commentType} onChange={(event) => setCommentType(event.target.value as CaseComment["type"])} aria-label="의견 유형"><option value="GENERAL">일반 의견</option>{role === "expert" && <><option value="EXPERT_OPINION">전문가 의견</option><option value="REQUEST_INFORMATION">추가 확인 필요</option><option value="SAFETY_WARNING">안전 주의</option></>}<option value="USER_EXPERIENCE">경험 공유</option>{role === "questioner" && <option value="RESOLUTION_UPDATE">해결 업데이트</option>}</select></div>
              {replyTo && <div className="replying-to">{replyTo.author}님의 의견에 답변 중<button type="button" onClick={() => setReplyTo(null)}><X /></button></div>}
              <textarea value={commentBody} onChange={(event) => setCommentBody(event.target.value)} placeholder={role === "expert" ? "가능한 원인, 추가로 확인할 정보, 안전 주의사항을 자유롭게 알려주세요." : "궁금한 점이나 비슷한 경험을 나눠주세요."} rows={5} />
              ...
              <div className="comment-form-footer"><button type="button" className="attach-button"><Paperclip />사진 첨부</button><span>{commentBody.length}/3000</span><Button type="submit" disabled={commentBody.trim().length < 2}><Send />의견 등록</Button></div>
            </form>
          </section>
        </article>

        <aside className="case-sidebar">
          <div className="sidebar-card status-card"><span className="eyebrow">현재 진행 상태</span><StatusBadge status={item.status} /><div className="status-progress"><span className="done" /><span className={item.status !== "OPEN" ? "done" : ""} /><span className={["REPAIR_REQUESTED", "IN_REPAIR", "RESOLVED"].includes(item.status) ? "done" : ""} /><span className={item.status === "RESOLVED" ? "done" : ""} /></div><div className="status-steps"><span>질문</span><span>진단</span><span>수리</span><span>해결</span></div><p>{item.status === "RESOLVED" ? "해결 결과가 공개 사례로 남았습니다." : "전문가와 사용자가 가능한 원인을 확인하고 있어요."}</p></div>
